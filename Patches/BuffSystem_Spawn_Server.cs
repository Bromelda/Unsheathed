...
namespace Unsheathed.Patches
{
    ...
    public static class BuffSystem_Spawn_Server_OnUpdate_Patch
    {
        ...
        static readonly PrefabGUID MovementBuff = new PrefabGUID(-822514423);
        ...
        static void BuffLog(string msg)
        {
            if (ConfigService.Debug_Buffs) Plugin.LogInstance.LogInfo(msg);
        }
        ...
        /// <summary>Rebuild the uncap allow-list from Spirit_Loadout (plus MovementBuff). Call after Spirit_ApplyAllConfigured().</summary>
        ...
        public static void AddUncapBuffGuid(PrefabGUID guid)
        {
            ...
            BuffLog($"[UncapAllowlist] Added {guid.GuidHash} at runtime.");
        }
        ...
        static void EnsureAllowlistBuilt()
        {
            ...
            _uncapBuffHashes.Add(MovementBuff.GuidHash); // always include MovementBuff
            ...
            BuffLog($"[UncapAllowlist] Built from Spirit_Loadout: count={_uncapBuffHashes.Count}");
        }
                    ...
                    if (owner != Entity.Null && em.Exists(owner) && em.HasComponent<PlayerCharacter>(owner))
                    {
                        ...
                        if (_uncapBuffHashes.Contains(h) && em.HasComponent<EntityOwner>(e))
                        {
                            if (_uncap.TryGetValue(owner, out var info))
                            {
                                info.RefCount++;
                                _uncap[owner] = info;
                                BuffLog($"[UncapWhileBuff] ++ref on {owner.Index} -> {info.RefCount}");
                            }
                            else
                            {
                                ...
                                if (hadCaps)
                                {
                                    ...
                                    BuffLog($"[UncapWhileBuff] Removed VampireAttributeCaps on char {owner.Index}.");
                                }
                            ...
                            // prevent double-strip: only strip here for equip buffs (MovementBuff already scheduled)
                            if (h != MovementBuff.GuidHash)
                                Core.StartCoroutine(RemoveSpawnScriptsNextFrame(e));
        ...
        static IEnumerator PatchVeilStormNextFrame(Entity buff,  float abilityAS, bool alsoStripSpawn)
        {
            ...
            BuffLog($"[PatchNextFrame] Patched AS on buff {buff.Index} ability={abilityAS}).");
        }
        ...
        static IEnumerator RemoveSpawnScriptsNextFrame(Entity buff)
        {
            ...
            BuffLog($"[StripSpawn] Removed ScriptSpawn (+extras) from buff {buff.Index}.");
        }
        ...
        static void AppendOrUpdate(ref DynamicBuffer<ModifyUnitStatBuff_DOTS> buf, UnitStatType type, float value)
        {
            for (int i = 0; i < buf.Length; i++)
            {
                ...
                buf[i] = m;
                BuffLog($"[AppendOrUpdate:UPDATED] stat={m.StatType} val={m.Value} cap={m.AttributeCapType} id={m.Id}");
                return;
            }
            ...
            buf.Add(added);
            BuffLog($"[AppendOrUpdate:ADDED] stat={added.StatType} val={added.Value} cap={added.AttributeCapType} id={added.Id}");
        }
        ...
        static IEnumerator RestoreCapsWhenBuffEnds(Entity owner, PrefabGUID buffGuid)
        {
            ...
            if (info.HadCaps)
            {
                ...
                BuffLog($"[UncapWhileBuff] Restored VampireAttributeCaps on char {owner.Index}.");
            }
            else
            {
                if (em.HasComponent<VampireAttributeCaps>(owner))
        ...
        static void DumpCapSourcesOnOwner(EntityManager em, Entity owner)
        {
            ...
            BuffLog($"[CapsDump] Owner={owner.Index} hasCaps={em.HasComponent<VampireAttributeCaps>(owner)}");
            ...
            if (em.HasBuffer<BuffBuffer>(owner))
            {
                ...
                for (int i = 0; i < buffs.Length; i++)
                {
                    ...
                    if (hasCaps || hasSpawn)
                    {
                        ...
                        BuffLog($"[CapsDump] BuffEntity={be.Index} guid={p.GuidHash} caps={hasCaps} spawn={hasSpawn}");
                    }
                }
            }
        }
